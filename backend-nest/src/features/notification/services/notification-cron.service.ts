    ...
    for (const task of candidates) {
      ...
      try {
        if (dueDate < startOfToday) {
          await this.notifyOnce(task.assignee.id, task.id, 'overdue', `Công việc "${task.title}" đã quá hạn`);
        } else if (dueDate <= endOfTomorrow) {
          await this.notifyOnce(task.assignee.id, task.id, 'due_soon', `Công việc "${task.title}" sắp đến hạn`);
        }
      } catch (err) {
